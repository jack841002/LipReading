...
class WordErrorRate(Callback):
	...
	def __init__(self, lipnet, val_generator: Sequence, decoder, samples: int = 50):
		super().__init__()
	...
	def calculate_wer(self, data: [tuple], label: [tuple], label_length: [tuple]) -> float:
		...
		for sen in label:
			temp = sen.split()
			word_length.append(len(temp))
		print(word_length)

		for i in range(len(data)):
			word_edit_distance = wer_sentence(str(data[i]), str(label[i]))
			words_num = words_num + word_length[i]

			if(word_edit_distance <= word_length[i]):
				word_error_num += word_edit_distance
			else:
				word_error_num += word_length[i]
	...
	def calculate_statistics(self) -> dict:
		...
		for i in batch_label:
			...
			batch_word_len.append(len(text.split(' ')))
