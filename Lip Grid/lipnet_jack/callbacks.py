...
class Statistics(keras.callbacks.Callback):
    ...
    def get_statistics(self, num):
        ...
        while num_left > 0:
            ...
            for j in range(0, num_proc):
                data.append(decoded_res[j])                                              # modify
    ...
    def get_mean_tuples(self, data, individual_length, func):
        ...
        for i in range(0, length):
            val         = float(func(data[i][0], data[i][1]))                             # modify
            total      += val
    ...
    '''def get_mean_bleu_score(self, data):
        wrapped_data = [([reference]) for reference in data]                              # modify
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        print('\n\n[Epoch %d] Out of %d samples: [CER: %.3f - %.3f] [WER: %.3f - %.3f]\n'
              % (epoch+1, stats['samples'], stats['cer'][0], stats['cer'][1], stats['wer'][0], stats['wer'][1]))    # modify
